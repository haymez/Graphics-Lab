...
function drawLine() {
	...
	canvas.addEventListener('click', function(evt) {
		...
		if (click == 1) {
			...
			toDraw[toDraw.length] = new makeLine(startX, startY, endX, endY);
			writeCode(); //TEST CODE! DELETE THIS!
		}
		...
		if (click % 2 == 1) {
			...
			if (click > 2) {
				if (findDistance(endX, endY, coor[0].startX, coor[0].startY) < 8) {
					this.removeEventListener('click',arguments.callee,false);
					toDraw[toDraw.length] = new makeLine(startX, startY, coor[0].startX, coor[0].startY);
					...
					toDraw[toDraw.length] = new makeLine(startX, startY, endX, endY);
					toDraw[toDraw.length-1].type = 'temp'; //Set this line to temperary becuase it's merely a preview
